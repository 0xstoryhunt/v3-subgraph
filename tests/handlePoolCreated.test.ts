...
import { convertTokenToDecimal, NULL_IP_HEX_STRING } from '../src/utils'
...
describe('handlePoolCreated', () => {
  ...
  test('success - create a pool', () => {
    ...
    assertObjectMatches('Factory', TEST_CONFIG.factoryAddress, [
      ...
      ['totalValueLockedIPUntracked', '0'],
      ['totalValueLockedUSDUntracked', '0'],
    ])

    assertObjectMatches('Bundle', '1', [['IPPriceUSD', '0']])
  ...
  test('success - skip pool creation if address in poolToSkip', () => {
    ...
    invokePoolCreatedWithMockedIPCalls(MOCK_EVENT, TEST_CONFIG_WITH_POOL_SKIPPED)
  ...
  describe('populateEmptyPools', () => {
    test('success', () => {
      const poolAddress = Address.fromString(USDC_WIP_03_MAINNET_POOL_FIXTURE.address)
      createMockedFunction(poolAddress, 'liquidity', 'liquidity():(uint128)').returns([
        ethereum.Value.fromUnsignedBigInt(BigInt.fromString(USDC_WIP_03_MAINNET_POOL_FIXTURE.liquidity)),
      ])
      createMockedFunction(poolAddress, 'fee', 'fee():(uint24)').returns([
        ethereum.Value.fromUnsignedBigInt(BigInt.fromString(USDC_WIP_03_MAINNET_POOL_FIXTURE.feeTier)),
      ])

      const token0Fixture = USDC_WIP_03_MAINNET_POOL_FIXTURE.token0
      const token0Address = Address.fromString(USDC_WIP_03_MAINNET_POOL_FIXTURE.token0.address)
      ...
      createMockedFunction(token0Address, 'balanceOf', 'balanceOf(address):(uint256)')
        .withArgs([ethereum.Value.fromAddress(Address.fromString(USDC_WIP_03_MAINNET_POOL_FIXTURE.address))])
        .returns([ethereum.Value.fromUnsignedBigInt(BigInt.fromString(token0Fixture.balanceOf))])

      const token1Fixture = USDC_WIP_03_MAINNET_POOL_FIXTURE.token1
      const token1Address = Address.fromString(USDC_WIP_03_MAINNET_POOL_FIXTURE.token1.address)
      ...
      populateEmptyPools(
        ...
        [
          [
            Address.fromString(USDC_WIP_03_MAINNET_POOL_FIXTURE.address), // first address is unused, hence reusing this pool address
            Address.fromString(USDC_WIP_03_MAINNET_POOL_FIXTURE.address),
            Address.fromString(USDC_WIP_03_MAINNET_POOL_FIXTURE.token0.address),
            Address.fromString(USDC_WIP_03_MAINNET_POOL_FIXTURE.token1.address),
          ],
        ],
        [],
        [],
      ...
      assertObjectMatches('Pool', USDC_WIP_03_MAINNET_POOL_FIXTURE.address, [
        ['token0', token0Fixture.address],
        ['token1', token1Fixture.address],
        ['feeTier', USDC_WIP_03_MAINNET_POOL_FIXTURE.feeTier],
        ['createdAtTimestamp', MOCK_EVENT.block.timestamp.toString()],
        ['createdAtBlockNumber', MOCK_EVENT.block.number.toString()],
        ['liquidityProviderCount', '0'],
        ['txCount', '0'],
  ...
  describe('fetchTokenSymbol', () => {
    ...
    test('success - fetch token symbol falls back to static definition', () => {
      ...
      createMockedFunction(usdcAddress, 'symbol', 'symbol():(string)').reverts()
      createMockedFunction(usdcAddress, 'symbol', 'symbol():(bytes32)').returns([
        ethereum.Value.fromBytes(Bytes.fromHexString(NULL_IP_HEX_STRING)),
      ])
